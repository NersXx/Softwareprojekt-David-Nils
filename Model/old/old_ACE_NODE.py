...
# Removed conflicting XLA flag that can force host platform device count and cause device mismatches
...
import LATENT_model as ace_node
...
snapshot = 50          # Speichere alle `snapshot` Epochen ein Modell
VECTOR_FIELD_DEPTH = 8      # Tiefe des MLP im Vector Field (3 = Input → Hidden → Hidden → Output)
...
CACHE_CLEAR_RSS_GB = int(os.environ.get("CACHE_CLEAR_RSS_GB", "100"))
...
def compute_global_stats(file_paths, input_dim, sample_size=10000):
    """
    Berechnet Mean und Std über eine zufällige Auswahl an Files im Streaming-Modus
    (Welford Online-Algorithmus). Vermeidet das Speichern großer Arrays im Speicher.
    """
    print("Berechne globale Normalisierungs-Statistiken (streaming)...")
    ...
    for i in indices:
        try:
            with np.load(file_paths[i], allow_pickle=True) as d:
            ...
            # Update running mean/std per row to keep memory footprint small
            for row in arr:
                count += 1
                delta = row - mean
                mean += delta / count
    ...
    if count == 0:
        ...
        print("Stats berechnet (Fallback: keine Daten).")
        return mean, std
...
class NPZDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            arr = np.zeros((10, self.input_dim), dtype=np.float32)
        ...
        # BUG FIX #1: Normalize BEFORE padding to avoid unnormalized zeros contaminating input
        ...
        # BUG FIX #2: Tighten label threshold to >= 0.5 for better separation
        # (> 0 is too permissive and treats all positive values as sepsis)
...
def run_evaluation(dataloader, n_devices, eval_step_pmap, params_repl, desc="Eval", write_preds_path=None, num_bins=1024, threshold=0.5):
    ...
    for batch in dataloader:
        ...
        # Force fixed global batch size to avoid pmap recompilation: pad to full device capacity
        target_global_bs = n_devices * BATCH_SIZE_PER_GPU
        if bx_np.shape[0] < target_global_bs:
            pad_size = target_global_bs - bx_np.shape[0]
...
def main():
    ...
    for epoch in range(current_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if len(seen_seq_lengths) > 1:
                ...
                # Avoid clearing JAX caches aggressively (causes recompiles). Run Python GC only.
                gc.collect()
            ...
            # Ensure computation completed to allow timely freeing of intermediate device buffers
            try:
            ...
            loss_cpu = float(loss_val[0].item())
            ...
            # Prevent dispatch queue growth: occasionally block on params so Python doesn't outpace GPU
            if i % 10 == 0:
                try:
                    jax.tree_map(lambda x: x.block_until_ready(), params_repl)
                except Exception:
                    pass

            # Free large temporaries from this iteration to avoid lingering references
            try:
                del bx_np, by_np, blast_np, bx_shard, by_shard, blast_shard
            except Exception:
                pass
            gc.collect()
        ...
        if (epoch + 1) % snapshot == 0:
            ...
            try:
                ...
                try:
                    del params_host_curr, params_unrep_curr, trained_model_curr
                except Exception:
                    pass
                gc.collect()
            except Exception as e:
        ...
        res = run_evaluation(test_loader, n_devices, eval_step_pmap, params_repl, desc=f"Test Ep {epoch+1}", write_preds_path=f"preds_epoch_{epoch+1}.csv")
        ...
        if triggered:
            # Avoid calling jax.clear_caches() inside the training loop (causes recompilation and fragmentation).
            print("Hinweis: Triggered memory guard - running python GC and pausing briefly (no jax.clear_caches()).")
            gc.collect()
            time.sleep(0.5)
            proc = psutil.Process(os.getpid())
            rss_mb_after = proc.memory_info().rss / (1024**2)
            rss_gb_after = rss_mb_after / 1024.0
            print(f"Nach GC: RSS host {rss_mb_after:.1f} MB ({rss_gb_after:.2f} GB)")

            if rss_gb_after >= CACHE_CLEAR_RSS_GB or (total_mem_gb is not None and rss_gb_after / float(total_mem_gb) >= CACHE_CLEAR_RATIO):
                print("WARNUNG: RSS bleibt hoch nach erstem GC, mache weiteren GC (no jax.clear_caches()).")
                gc.collect()
                time.sleep(0.5)
                proc = psutil.Process(os.getpid())
    ...
    print("\n--- FINAL SUMMARY ---")
    run_evaluation(test_loader, n_devices, eval_step_pmap, params_repl, desc="FINAL TEST SET", write_preds_path="preds_final.csv")
