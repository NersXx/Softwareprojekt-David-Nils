...
import ASS4.ACE_model as ace_node
...
def make_fns(model_static, optimizer):
    ...
    def loss_fn(params, model_static, x, y, last_idxs, keys):
        ...
        """
        # keys hat Shape (local_batch_size, 2) da pmap mit in_axes=0 die erste Dim mapped
        # Nutze diese keys direkt für die vmap (ein Key pro Sample)
...
def main():
   ...
   # In main() deiner ACE_NODEoptimized_v3.py:
    ...
    # Globaler Step Counter für Learning Rate Tracking
    global_step = 0

    print(f"Start Training: Peak LR={current_lr}, Hidden={HIDDEN_DIM}")
    ...
    for epoch in range(current_epochs):
        ...
        # 6. Test Evaluation nach jedem Epoch
        res = run_evaluation(test_loader, n_devices, eval_step_pmap, params_repl, desc=f"Test Ep {epoch+1}")
