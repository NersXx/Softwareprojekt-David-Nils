...
class ACE_VectorField(eqx.Module):
    ...
    def __init__(self, hidden_dim: int, *, key, vector_field_depth: int = 3, vector_field_width: int = 64):
        ...
        self.f_ode = MLP(in_size=hidden_dim + 1, out_size=hidden_dim, 
                         width_size=vector_field_width, depth=vector_field_depth, key=k_f)
        ...
        self.g_ode = MLP(in_size=hidden_dim + hidden_dim, out_size=hidden_dim,
                         width_size=vector_field_width, depth=vector_field_depth, key=k_g)

    def __call__(self, t: float, y: jnp.ndarray, args) -> jnp.ndarray:
        """
        Combines two ODE flows:
        - dy_f: General temporal evolution
        - dy_g: Attention-modulated evolution
        
        Args:
            t: Current time
            y: Current state (hidden_dim,)
            args: Flattened attention matrix (hidden_dim^2,)
        
        Returns:
            dy: Combined dynamics (hidden_dim,)
        """
        # --- Model 1: General Flow (f_ode) ---
        t_arr = jnp.array([t], dtype=jnp.float32)
        mlp_in_f = jnp.concatenate([y, t_arr], axis=0)
        dy_f = self.f_ode(mlp_in_f)
        
        # --- Model 2: Attention/Context Flow (g_ode) ---
        # Reshape the flat attention vector into matrix A
        attn_flat = args
        A = attn_flat.reshape(self.hidden_dim, self.hidden_dim)
        
        # Compute attention modulation: A @ y (hidden_dim,)
        attn_mod = jnp.dot(A, y)
        
        # Pass both state and attention modulation to g_ode
        mlp_in_g = jnp.concatenate([y, attn_mod], axis=0)
        dy_g = self.g_ode(mlp_in_g)
...
class ACE_NODE(eqx.Module):
    ...
    def __init__(self, hidden_dim: int, *, key, input_dim: int = 40, vector_field_depth: int = 3, vector_field_width: int = 64):
        """
        input_dim defaults to 40 to match ACE_NODE.py.
