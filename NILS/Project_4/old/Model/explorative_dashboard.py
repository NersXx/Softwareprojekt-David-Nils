...
def mean_confusion_last_epoch(
    ...
    for run in all_runs:
        ...
        for key in percents.keys():
            source_key = f"{key}_count" if f"{key}_count" in metrics else key
            last_counts[key] = last_finite(metrics.get(source_key, np.array([])))
        total = sum(last_counts.values())
        if not np.isfinite(total) or total == 0:
            continue
        for key, value in last_counts.items():
            percents[key].append(value / total)
    means = {}
    for key, values in percents.items():
        arr = np.asarray(values, dtype=float)
        mean = np.nan if arr.size == 0 else float(np.nanmean(arr))
        means[key] = mean
    return means["tp"], means["tn"], means["fp"], means["fn"]


def make_confusion_tables(
    out_dir: Path,
    all_runs: List[RunInfo],
    metrics_by_run: Dict[Path, Dict[str, np.ndarray]],
) -> List[str]:
    ...
    created: List[str] = []
    for model in models:
        mean_tp, mean_tn, mean_fp, mean_fn = mean_confusion_last_epoch(model, all_runs, metrics_by_run)
        def format_pct(value: float) -> str:
            return "nan" if not np.isfinite(value) else f"{value * 100.0:.2f}%"
        table_data = [
            ["", "Pred +", "Pred -"],
            ["Actual +", format_pct(mean_tp), format_pct(mean_fn)],
            ["Actual -", format_pct(mean_fp), format_pct(mean_tn)],
        ]
        ...
        table.scale(1.0, 1.2)
        ax.set_title(f"{model} confusion (%)", pad=8)
...
def build_html(
    ...
    html_lines += [
        ...
        "  </div>",
        "  <div class=\"section\">",
        "    <h2>Mean Confusion Percentages (last epoch)</h2>",
        "    <div class=\"grid\">",
    ]
    for name in confusion_tables:
...
def main() -> int:
    ...
    confusion_tables = make_confusion_tables(args.out_dir, runs, metrics_by_run)
    ...
    build_html(args.out_dir, runs, metrics_by_run, confusion_tables, boxplots, mean_curves, summary_file)
