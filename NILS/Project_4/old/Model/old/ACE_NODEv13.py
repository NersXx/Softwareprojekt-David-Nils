...
# ---------------------------------------------------------
def list_npz_files(index_path="npz_dir/index.csv", shuffle=False, seed=42):
    df = pd.read_csv(index_path)
    paths = df["npz_path"].tolist()
    labels_all = df["label"].tolist() if "label" in df.columns else [None] * len(paths)
    ...
    train_files  = paths[:n_train]; train_labels = labels_all[:n_train]
    val_files    = paths[n_train:n_train + n_val]; val_labels = labels_all[n_train:n_train + n_val]
    test_files   = paths[n_train + n_val:]; test_labels = labels_all[n_train + n_val:]

    print(f"Counts: train={len(train_files)}, val={len(val_files)}, test={len(test_files)}")
    return df, train_files, train_labels, val_files, val_labels, test_files, test_labels
...
def main():
    ...
    # Index laden
    # Index und Splits laden (funktion gibt: df, train_files, train_labels, val_files, val_labels, test_files, test_labels)
    df, train_files, train_labels, val_files, val_labels, test_files, test_labels = list_npz_files("npz_dir/index.csv")

    # Für den bisherigen Code, der 'files' und 'labels_csv' erwartet, setze 'files' auf die Trainingsliste:
    files = train_files
