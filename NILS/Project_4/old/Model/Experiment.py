...
INPUT_DIM = 80
HIDDEN_DIM = 32 
BATCH_SIZE_PER_GPU = 100       # wenn cpu, dann gesamte Batch Size
Epochen = 40
Warmup_epochs = 5
Learning_Rate = 1e-3
Final_LR = 1e-9
...
VECTOR_FIELD_WIDTH = 50     # Breite der Hidden Layers im Vector Field MLP
...
def compute_global_stats(file_paths, input_dim, sample_size=10000):
    ...
    if not buffer:
        raise ValueError(
            "No valid NPZ arrays loaded for stats. Check index.csv paths and file existence."
        )
...
def list_npz_files_split(index_path=None, labels_path=None):
    # Prefer the configured base path so relative paths are stable
    base_dir = Path(CONFIG_PATH).parent
    if index_path is None:
        index_path = base_dir / "npz_dir" / "index.csv"
    if labels_path is None:
        labels_path = base_dir / "B" / "labels.csv"
    index_path = Path(index_path)
    labels_path = Path(labels_path)
    df = pd.read_csv(index_path)

    # Resolve relative NPZ paths against the base dir (files/)
    resolved_paths = []
    for p in df["npz_path"]:
        p_path = Path(p)
        if not p_path.is_absolute():
            cand_base = (base_dir / p_path).resolve()
            cand_index = (index_path.parent / p_path).resolve()
            p_path = cand_base if cand_base.exists() else cand_index
        resolved_paths.append(str(p_path))
    df["npz_path"] = resolved_paths

    # Optional: override labels from labels.csv in files/B
    if labels_path.exists():
        labels_df = pd.read_csv(labels_path)
        if "filename" in labels_df.columns:
            labels_df = labels_df.rename(columns={"filename": "pid"})
        labels_df = labels_df[["pid", "label"]].copy()
        labels_df["pid"] = labels_df["pid"].astype(str)
        df["pid"] = df["pid"].astype(str)
        df = df.merge(labels_df, on="pid", how="left", suffixes=("", "_labels"))
        if "label_labels" in df.columns:
            df["label"] = df["label_labels"].combine_first(df.get("label"))
            df = df.drop(columns=["label_labels"])
